...
const AdminOrders = () => {
  ...
  const { data: orders, isLoading, isError, error } = RepoGetAllOrders();
  ...
  const columns = [
    {
      ...
      renderCell: (index) =>{ return (
        <Box
          display="flex"
        >
          <Typography color={colors.grey[100]} sx={{ ml: "0px" }}component={Link} to={`../adminorderdetails/${orders[index.api.getRowIndex(index.row["_id"])]["_id"]}`}  >
